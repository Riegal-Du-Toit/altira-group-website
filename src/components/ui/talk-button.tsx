...
export function TalkButton({ href = TALK_TARGET, children = "BOOK A DEMO" }: { href?: string; children?: ReactNode }) {
  ...
  return (
    <a href={href} onClick={handleClick} className={`talk-button relative inline-flex items-stretch overflow-hidden rounded-[12px] border-[1.5px] border-[#F7F8FA] bg-[#37D8C6] p-0 text-[16px] font-bold text-white shadow-[0_8px_18px_rgba(17,22,61,0.18)] transition-all duration-300 ease-out hover:bg-[#2fc7b7] active:scale-[0.97] ${openSansThin.className}`}>
      <span className="relative flex items-center gap-1.5 rounded-[10px] bg-white/10 px-[1.05em] py-[0.64em] pr-[0.95em] text-[0.8rem] font-light !text-white transition-colors duration-300 sm:text-[0.88rem]">
        <span>{children}</span>
