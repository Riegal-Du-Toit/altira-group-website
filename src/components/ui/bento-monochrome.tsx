...
export default function BentoMonochrome() {
  ...
  return (
    <section ref={sectionRef} className={`relative left-1/2 w-screen -translate-x-1/2 overflow-hidden bg-transparent text-white transition-colors duration-500 ${visible ? "" : "motion-safe:opacity-0"}`}>
      <style>{`
      ...
      <div className={`relative mx-auto w-full max-w-none px-6 py-20 motion-safe:translate-y-40 md:px-10 lg:px-[max(7.75rem,calc((100vw-96rem)/2+7.75rem))] ${visible ? "motion-safe:animate-[bento2-intro_.9s_ease-out_forwards]" : ""}`}>
        <header className="mb-10 flex flex-col gap-6 pb-6 md:flex-row md:items-end md:justify-between">
          <div className="flex flex-col gap-2">
            <span className="text-xs uppercase tracking-[.35em] text-white/60">Why Altira</span>
          ...
          <p className="max-w-sm text-sm text-white/72 md:text-base">Altira Group builds the technology and customer journey that make your product easier to sell and easier to use.</p>
        </header>
        <FinancialScoreCards />
      </div>
    </section>
